...
Contribution to `uaparser_gleam` is encouraged: bug reports, discussions,
...
`uaparser_gleam` is governed under the
...
`uaparser_gleam` contributions must be well understood by the submitter and that
...
`uaparser_gleam` projects have adopted a variation of the Conventional Commits
...
Austin Ziegler created `uaparser_gleam`.
